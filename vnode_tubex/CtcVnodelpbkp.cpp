...
namespace tubex {
    ...
    void CtcVnodelp::Contract(vnodelp::AD *ad, double t, double tend, int n, Tube &x,bool ode_type, double initial_time) {

        ibex::Interval domain = ibex::Interval(t, tend);
        TubeVector y(domain);
        y[0] = x;
        Contract( ad, t, tend, n, y,ode_type, initial_time);
        x=y[0];
    }//

    void CtcVnodelp::Contract(vnodelp::AD *ad, double t, double tend, int n, TubeVector &x,bool ode_type, double initial_time) {
...
//        TubeVector bwd_x(domain, n);
        ...
        //initialisation of vstate (get time discretiziation of x and gates)
        Vstate gates_vector;
...
//                cout << "time : " << gates_vector[i].first << " - > [ " << gates_vector[i].second << " ] --> vol : " << gates_vector[i].second.volume()
			...
			min_air+=gates_vector[min_starter_index].second[j].diam();
			}	
...
//            cout << "min vol index : " << min_starter_index << "- > " << gates_vector[min_starter_index].first<<" : " <<gates_vector[min_starter_index].second << endl;
        ...
        tempo_x.set(gates_vector[min_starter_index].second, gates_vector[min_starter_index].first);
...
/* to be  tested
        ...
        if(ode_type == 1) {
            for (int i = 0; i < time_gate.size(); i++) {
                gates_vector.emplace_back(time_gate[i], si[i]);
                cout << "time : " << s[i].first << " - > [ " << gates_vector[i].second << " ] --> diam : " << gates_vector[i].second.min_diam()<< endl;
                if (i == 0)
                    min_starter_index = i;
                else {
                    if (gates_vector[i].second.min_diam() < gates_vector[min_starter_index].second.min_diam())
                        min_starter_index = i;
                }
            }

            cout << "min diam index : " << min_starter_index << "- > " << gates_vector[min_starter_index].second << endl;
        }
...
*/
    ...
    //    cout << "# "<< min_starter_index << " ## "<<gates_vector.size()<< endl;
        if (min_starter_index<gates_vector.size()-1) {
            //fwd_x.set(gates_vector[min_starter_index].second, gates_vector[min_starter_index].first);
            // integration parameters

            //time
            interval ti = interval(gates_vector[min_starter_index].first);
            int k=1;
            interval tnext = interval(gates_vector[min_starter_index + k].first);

            //initial condition
            iVector y(n);
            for (int i = 0; i < n; i++) {
                y[i] = interval(gates_vector[min_starter_index].second[i].lb(),
                                gates_vector[min_starter_index].second[i].ub());
            }
		...
		vnode_fwd_solver->setHmin(0.0039);
...
//                cout << "k "<< k << endl;
                ...
                tnext=gates_vector[min_starter_index + k].first;
       ...
       // cout << "# "<< min_starter_index << " ## "<<gates_vector.size()<< endl;
        if (min_starter_index>0) {
           ...
           // bwd_x.set(gates_vector[min_starter_index].second, gates_vector[min_starter_index].first);
            ...
            interval ti = interval(gates_vector[min_starter_index].first);
            int k=-1;
            interval tprev = interval(gates_vector[min_starter_index + k].first);
		...
		vnode_bwd_solver->setHmin(0.0039);
            //main integration loop
            while(ti!=interval(t)){
                //"slice" integration loop
                while(ti!=tprev){
  ...
  //                  for (int ith_sol=0; ith_sol<n; ith_sol++)
    //                    cout <<"solution enclosure at t = " << inf(ti) << " ["<<inf(y[ith_sol]) << ", " <<sup(y[ith_sol])<<"]"<<endl;
                ...
                k--;
      //          cout << "k "<< k << endl;
                if(min_starter_index + k>=0)
                    tprev=gates_vector[min_starter_index + k].first;
