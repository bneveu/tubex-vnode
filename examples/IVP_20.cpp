...
void ivp20(int n, var_type*yp, const var_type*y, var_type t, void*param)
{
 interval p=interval(0.1); interval s=interval (1.);
 yp[0]=-y[1] + p*y[0]*(s-sqr(y[0]) - sqr(y[1])) ;
    yp[1]=y[0]+ p*y[0]*(s-sqr(y[0]) - sqr(y[1])) ;
}
...
void contract(TubeVector& x, double t0, bool incremental)
{
    ...
    AD *ad=new FADBAD_AD(n,ivp20,ivp20);
    CtcVnodelp c;
...
{   float temps;
    ...
    IntervalVector v(2);
    v[0]=Interval(0.9,1.1);
    v[1]=Interval(-0.1,0.1);
